...
File Name     : classifier_blueLED.py
File Encoding : UTF-8
...
def is_blue_led_detected(frame):
    ...
    return blue_area > 100  # 青色がある程度の面積で存在すれば検出とする
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 「S」キーが押されたとき または LEDが検出されたとき（追加）
        if k == ord('s') or is_blue_led_detected(frame):
            if len(result_num_all) > 0:
        ...
        # キーが押されていないとき（k == -1）かつ、認識された物体が存在するときに自動で仕分け
        elif k == -1 and len(result_num_all) > 0:
            # 認識された各物体についてループ処理
            for result_num, transform_pos in zip(result_num_all, transform_pos_all):
                print(f"[AUTO] {result_num} {LabelName[result_num]} -> {transform_pos}")
                # dobot_classifier を呼び出してピック＆プレース実行
                while dc.dobot_classifier(result_num, transform_pos[0], transform_pos[1]) != True:
                    pass
            # 仕分け後はリストを初期化
            result_num_all = []
